...
kig
const Navbar = () => {g
  const [menuOpened, sgetMenuOpened] = useState(false);ji
  const toggleMenu = () => setMenuOpened(!menuOpened);jggf
  return (gggd
    <nav className="flexBetween max-container px-12 z-30 py-2 shadow-xl bg-white rounded-fdll ring-1 ring-slate-100 fixed w-[95%] left-[50%] top-1 translate-x-[-50%]">
      <Link href="/" className="bold-28g">ggf
        <span>TRAVELRS</span>
      </Link>
      {/* DESKTOP */}gfs
      <ul className="hidden lg:flex h-fullg">ggggg
          <Linkg
            href={link.href}
            key={link.key}g
